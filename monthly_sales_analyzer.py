...
for i in range(len(sales_data)):
    sales_data[i]["total"] = sales_data[i]["product_a"] + sales_data[i]["product_b"] + sales_data[i]["product_c"]
...
def best_selling_day(data):
    ...
    best_day = 0
    total_sales = data[best_day]["total"]
    
    for i in range(len(data)):
        if data[i]["total"] > total_sales:
            best_day = i
            total_sales = data[best_day]["total"]
...
def days_above_threshold(data, product_key, threshold):
    ...
    num_dats_gt_threshold = 0
    for i in range(len(data)):
        if data[i][product_key] > threshold:
            num_dats_gt_threshold = num_dats_gt_threshold + 1

    return num_dats_gt_threshold
...
def worst_selling_day(data):
    ...
    worst_day = 0
    total_sales = data[worst_day]["total"]
    
    for i in range(len(data)):
        if data[i]["total"] < total_sales:
            worst_day = i
            total_sales = data[worst_day]["total"]
    return data[worst_day]['day']
